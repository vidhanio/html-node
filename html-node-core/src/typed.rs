...
pub trait TypedElement: Default {
    ...
    /// Convert the element into a list of attributes.
    fn into_attribute_list(self) -> Vec<(String, Option<String>)>;
}

/// Helper macro to define a typed HTML element.
#[macro_export]
macro_rules! typed_element {
    ...
    (@ $Name:ident[$name:literal] $Attributes:ident { $($attribute:ident),* $(,)? }) => {
        ...
        impl $crate::TypedElement for $Name {
            ...
            fn into_attribute_list(self) -> ::std::vec::Vec<(::std::string::String, ::std::option::Option<::std::string::String>)> {
                use ::std::iter::Iterator as _;

                [$((stringify!($attribute), self.attributes.$attribute)),*]
                    .into_iter()
